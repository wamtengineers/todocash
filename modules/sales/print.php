...
doquery( "update sales set is_printed = 1 where id='".slash($_GET["id"])."'", $dblink );
if( $sale[ "is_printed" ] == "1" && $_SESSION[ "logged_in_admin" ][ "admin_type_id" ] != "1" ) {
	die( "Already printed" );
}
?>
<!DOCTYPE html>
...
p{margin:0 0 3px 0; font-size:9px;}
#logo {
	text-align: center;
	margin-bottom: 10px;
}
...
<div id="main">
    <div id="logo">
    	<img src="<?php echo $file_upload_root;?>config/<?php echo get_config("reciept_logo");?>" style="width:65%" />
    	<span style="display:block; font-size:10px;"><?php echo nl2br(get_config("address_phone"))?></span>
    </div>
    <div style="display:none" id="receipt">RECEIPT</div>
    <?php
    ...
    <div class="contentbox">
        ...
        <table cellpadding="0" cellspacing="0" align="center" width="800" border="0" class="items">
            ...
            if(numrows($items)>0){
                ...
                while($item=dofetch($items)){
                    ?>
                    <tr>
                    	<td style="text-align:center; font-size:9px;"><?php echo $sn++?></td>
                        <td style="text-align:left; font-size:9px;"><?php echo unslash($item["title"])?></td>
        ...
        <hr style="border:0; border-top:1px solid #999; margin:3px 0">
    ...
    <div id="signcompny" style="font-size:9px;">Software developed by wamtSol - 0346 3891 662</div> 
</div>
</body>
</html>
<?php
